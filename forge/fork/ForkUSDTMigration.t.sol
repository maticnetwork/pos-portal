...
contract ForkUSDTMigration is Test {
    ...
    function test_exit_disabled() public {
        // Disable USDT exit
        uint256 hardcodedLastExitBlockNumber = 53973586; // Hardcoded value in the input data for testing
        string memory input =
        ...
        // Manually map the usdt to child token to bypass the TOKEN_NOT_MAPPED error
        address tempChildUSDT = address(0x0165878A594ca255338adfa4d48449f69242Eb8F);
        bytes32 newChildToRootSlot = keccak256(abi.encode(tempChildUSDT, uint256(4)));
        vm.store(address(rootChainManagerProxy), newChildToRootSlot, bytes32(uint256(uint160(address(usdt)))));
        ...
        // Reenable USDT exit
        input = _getUpdateTokenMigrationStatusInputs(address(usdt), true, false, 0);
        callData = updateTokenMigrationStatusScript.run(input);
        vm.prank(address(safeMultisig));
        (success,) = address(rootChainManagerProxy).call(callData);
        assertTrue(success, "Failed to reenable USDT exit");
