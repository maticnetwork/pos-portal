...
contract('RootChainManager', async(accounts) => {
  ...
  describe('Withdraw ERC20 :: withdraw for another account', async() => {
    const depositAmount = mockValues.amounts[1]
    let totalDepositedAmount = new BN('0')
    const withdrawAmount = mockValues.amounts[1]
    const depositReceiver = accounts[0]
    ...
    it('Can receive withdraw tx', async() => {
      withdrawTx = await contracts.child.dummyERC20.withdrawFor(withdrawReceiver, withdrawAmount, { from: depositReceiver })
      should.exist(withdrawTx)
    })

    it('Should emit Transfer log in withdraw tx', () => {
      const logs = logDecoder.decodeLogs(withdrawTx.receipt.rawLogs)
      transferLog = logs.filter(l => l && l.event === 'Transfer').pop()
      should.exist(transferLog)
    })
    ...
    it('Should have more amount in withdrawer account after withdraw', async() => {
      const newWithdrawAccountBalance = await dummyERC20.balanceOf(withdrawReceiver)
      newWithdrawAccountBalance.should.be.a.bignumber.that.equals(
        depositAmount
      )

      const newDepositAccountBalance = await dummyERC20.balanceOf(depositReceiver)
  ...
  describe('Withdraw ERC721 :: withdraw for another account', async() => {
    const depositTokenId = mockValues.numbers[4]
    const depositAmount = new BN('1')
    const withdrawAmount = new BN('1')
    const depositReceiver = accounts[0]
    ...
    it('Can receive withdraw tx', async() => {
      withdrawTx = await contracts.child.dummyERC721.withdrawFor(withdrawReceiver, depositTokenId, { from: depositReceiver })
      should.exist(withdrawTx)
    })
    ...
    it('Should fail to start exit again (change the log index to generate same exit hash)', async() => {
      const logIndex = toHex(Array(63).fill(0).join('') + '3')
      const data = bufferToHex(
        rlp.encode([
          headerNumber,
          bufferToHex(Buffer.concat(checkpointData.proof)),
    ...
    it('Should have more amount in withdrawer account after withdraw', async() => {
      const newWithdrawAccountBalance = await dummyERC721.balanceOf(withdrawReceiver)
      newWithdrawAccountBalance.should.be.a.bignumber.that.equals(
        depositAmount
      )

      const newDepositAccountBalance = await dummyERC721.balanceOf(depositReceiver)
      newDepositAccountBalance.should.be.a.bignumber.that.equals(
        accountBalance
      )

      const owner = await dummyERC721.ownerOf(depositTokenId)
      owner.should.equal(withdrawReceiver)
    })
  ...
  describe('Withdraw batch ERC721 :: withdraw for another account', async() => {
    const tokenId1 = mockValues.numbers[4]
    const tokenId2 = mockValues.numbers[5]
    const tokenId3 = mockValues.numbers[8]
    const user = accounts[0]
    ...
    it('User should be able to start withdraw', async() => {
      withdrawTx = await childToken.withdrawForBatch(beneficiary, [tokenId1, tokenId2, tokenId3])
      should.exist(withdrawTx)
    })
    ...
    it('User should own tokens on root chain', async() => {
      {
        const owner = await rootToken.ownerOf(tokenId1)
        owner.should.equal(beneficiary)
      }
      {
        const owner = await rootToken.ownerOf(tokenId2)
        owner.should.equal(beneficiary)
  ...
  describe('Withdraw single ERC1155 :: withdraw for another account', async() => {
    const tokenId = mockValues.numbers[8]
    const depositAmount = mockValues.amounts[1]
    const withdrawAmount = mockValues.amounts[1]
    const depositReceiver = accounts[0]
    ...
    it('Can receive withdraw tx', async() => {
      withdrawTx = await contracts.child.dummyERC1155.withdrawForSingle(withdrawReceiver, tokenId, withdrawAmount, { from: depositReceiver })
      should.exist(withdrawTx)
    })

    it('Should emit Transfer log in withdraw tx', () => {
      const logs = logDecoder.decodeLogs(withdrawTx.receipt.rawLogs)
      transferLog = logs.filter(l => l && l.event === 'TransferSingle').pop()
      should.exist(transferLog)
    })
    ...
    it('Should have more amount in withdrawer account after withdraw', async() => {
      const newWithdrawAccountBalance = await dummyERC1155.balanceOf(withdrawReceiver, tokenId)
      newWithdrawAccountBalance.should.be.a.bignumber.that.equals(
        depositAmount
      )

      const newDepositAccountBalance = await dummyERC1155.balanceOf(depositReceiver, tokenId)
      newDepositAccountBalance.should.be.a.bignumber.that.equals(
        accountBalance
      )
    })
  ...
  describe('Withdraw batch ERC1155 :: withdraw for another account', async() => {
    let erc1155PredicateAddress
    const withdrawAmountA = mockValues.amounts[2]
    const withdrawAmountB = mockValues.amounts[2]
    const withdrawAmountC = mockValues.amounts[1]
    ...
    it('Can receive withdraw tx', async() => {
      withdrawTx = await contracts.child.dummyERC1155.withdrawForBatch(
        withdrawReceiver,
        [tokenIdA, tokenIdB, tokenIdC],
        [withdrawAmountA, withdrawAmountB, withdrawAmountC],
        { from: depositReceiver })
    ...
    it('Should emit Transfer log in withdraw tx', () => {
      logs = logDecoder.decodeLogs(withdrawTx.receipt.rawLogs)
      transferBatchLog = logs.filter(l => l && l.event === 'TransferBatch').pop()
      should.exist(transferBatchLog)
    })
    ...
    it('Should have more amount in withdrawer account after withdraw', async() => {
      const newWithdrawAccountBalanceA = await dummyERC1155.balanceOf(withdrawReceiver, tokenIdA)
      newWithdrawAccountBalanceA.should.be.a.bignumber.that.equals(withdrawAmountA)
      const newWithdrawAccountBalanceB = await dummyERC1155.balanceOf(withdrawReceiver, tokenIdB)
      newWithdrawAccountBalanceB.should.be.a.bignumber.that.equals(withdrawAmountB)
      const newWithdrawAccountBalanceC = await dummyERC1155.balanceOf(withdrawReceiver, tokenIdC)
      newWithdrawAccountBalanceC.should.be.a.bignumber.that.equals(withdrawAmountC)

      const newDepositAccountBalanceA = await dummyERC1155.balanceOf(depositReceiver, tokenIdA)
      newDepositAccountBalanceA.should.be.a.bignumber.that.equals(accountBalanceA)

      const newDepositAccountBalanceB = await dummyERC1155.balanceOf(depositReceiver, tokenIdB)
      newDepositAccountBalanceB.should.be.a.bignumber.that.equals(accountBalanceB)

      const newDepositAccountBalanceC = await dummyERC1155.balanceOf(depositReceiver, tokenIdC)
      newDepositAccountBalanceC.should.be.a.bignumber.that.equals(accountBalanceC)
    })
