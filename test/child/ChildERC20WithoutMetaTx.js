...
contract('ChildERC20WithoutMetaTx', (accounts) => {
  describe('Should mint tokens on deposit', () => {
    ...
    before(async() => {
      ...
      oldAccountBalance = await contracts.dummyERC20WithoutMeta.balanceOf(depositReceiver)
      const DEPOSITOR_ROLE = await contracts.dummyERC20WithoutMeta.DEPOSITOR_ROLE()
      await contracts.dummyERC20WithoutMeta.grantRole(DEPOSITOR_ROLE, accounts[0])
    })

    it('Can receive deposit tx', async() => {
      depositTx = await contracts.dummyERC20WithoutMeta.deposit(depositReceiver, depositData)
    ...
    describe('Correct values should be emitted in Transfer log', () => {
      it('Event should be emitted by correct contract', () => {
        transferLog.address.should.equal(
          contracts.dummyERC20WithoutMeta.address.toLowerCase()
        )
      })
    ...
    it('Deposit amount should be credited to deposit receiver', async() => {
      const newAccountBalance = await contracts.dummyERC20WithoutMeta.balanceOf(depositReceiver)
  ...
  describe('Deposit called by non depositor account', () => {
    ...
    before(async() => {
      ...
      dummyERC20 = contracts.dummyERC20WithoutMeta
    })
  ...
  describe('Should burn tokens on withdraw', () => {
    ...
    before(async() => {
      ...
      await contracts.dummyERC20WithoutMeta.grantRole(DEPOSITOR_ROLE, accounts[0])
      const depositData = abi.encode(['uint256'], [depositAmount.toString()])
      await contracts.dummyERC20WithoutMeta.deposit(user, depositData)
      oldAccountBalance = await contracts.dummyERC20WithoutMeta.balanceOf(user)
    })

    it('Can receive withdraw tx', async() => {
      withdrawTx = await contracts.dummyERC20WithoutMeta.withdraw(withdrawAmount)
    ...
    it('Withdraw amount should be deducted from user', async() => {
      const newAccountBalance = await contracts.dummyERC20WithoutMeta.balanceOf(user)
      newAccountBalance.should.be.a.bignumber.that.equals(
