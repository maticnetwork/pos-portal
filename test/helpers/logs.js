...
export const getERC20TransferLog = ({ overrideSig, from, to, amount }) => {
  return encodeRlp(['0x', [overrideSig || erc20TransferEventSig, from, to], toBeHex(amount)])
}

export const getERC721TransferLog = ({ overrideSig, from, to, tokenId }) => {
  return encodeRlp(['0x', [overrideSig || erc721TransferEventSig, from, to, toBeHex(tokenId)]])
}

export const getERC721WithdrawnBatchLog = ({ overrideSig, user, tokenIds }) => {
  return encodeRlp([
    '0x',
    [overrideSig || erc721WithdrawnBatchEventSig, user],
    abi.encode(['uint256[]'], [tokenIds.map((t) => '0x' + t.toString(16))])
  ])
}

export const getERC721TransferWithMetadataLog = ({ overrideSig, from, to, tokenId, metaData }) => {
  return encodeRlp([
    '0x',
    [overrideSig || erc721TransferWithMetadataEventSig, from, to, '0x' + tokenId.toString(16)],
...
export const getERC1155ChainExitLog = ({ overrideSig, to, tokenIds, amounts, data }) => {
  return encodeRlp([
    '0x',
