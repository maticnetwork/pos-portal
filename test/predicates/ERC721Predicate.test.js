...
contract('ERC721Predicate', (accounts) => {
  describe('lockTokens', () => {
    ...
    it('Should be able to receive lockTokens tx', async () => {
      ...
      await expect(
        erc721Predicate.lockTokens(depositor.address, depositReceiver, dummyERC721.target, depositData)
  ...
  describe('lockTokens called by non-manager', () => {
    ...
    it('Should revert with correct reason', async () => {
      const depositData = abi.encode(['uint256'], [tokenId]);
      await expect(
        erc721Predicate.connect(depositor).lockTokens(depositor.address, depositReceiver, dummyERC721.target, depositData)
  ...
  describe('exitTokens failing with WithdrawnBatch, while passing with Transfer', () => {
    ...
    it('exitTokens should revert with WithdrawnBatch event', async () => {
      ...
      await expect(
        erc721Predicate.exitTokens(dummyERC721.target, dummyERC721.target, burnLog)
      ).to.be.revertedWith('ERC721Predicate: INVALID_SIGNATURE');
    })
  ...
  describe('exitTokens called using normal transfer log instead of burn', () => {
    ...
    it('Should revert with correct reason', async () => {
      ...
      await expect(
        erc721Predicate.exitTokens(dummyERC721.target, dummyERC721.target, burnLog)
      ).to.be.revertedWith('ERC721Predicate: INVALID_RECEIVER');
    })
  })
  ...
  describe('exitTokens called by non manager', () => {
    ...
    it('Should revert with correct reason', async () => {
      ...
      await expect(
        erc721Predicate.connect(await ethers.getSigner(accounts[2])).exitTokens(dummyERC721.target, dummyERC721.target, burnLog)
      ).to.be.revertedWith('ERC721Predicate: INSUFFICIENT_PERMISSIONS');
    })
  })
})
