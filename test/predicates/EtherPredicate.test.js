...
contract('EtherPredicate', (accounts) => {
  describe('lockTokens', () => {
    ...
    it('Should be able to receive lockTokens tx', async () => {
      ...
      await expect(
        etherPredicate.lockTokens(depositor.address, depositReceiver, etherAddress, depositData)
  ...
  describe('exitTokens', () => {
    ...
    it('Should be able to receive exitTokens tx', async () => {
      ...
      await expect(
        etherPredicate.exitTokens(etherPredicate.target, etherAddress, burnLog)
  ...
  describe('exitTokens with incorrect burn transaction signature', () => {
    ...
    it('Should revert with correct reason', async () => {
      ...
      await expect(
        etherPredicate.exitTokens(etherPredicate.target, etherAddress, burnLog)
      ).to.be.revertedWith('EtherPredicate: INVALID_SIGNATURE');
    })
  })
  ...
  describe('exitTokens called using normal transfer log instead of burn', () => {
    ...
    it('Should revert with correct reason', async () => {
      ...
      await expect(
        etherPredicate.exitTokens(etherPredicate.target, etherAddress, burnLog)
      ).to.be.revertedWith('EtherPredicate: INVALID_RECEIVER');
    })
  })
