...
export async function processTemplates() {
  try {
    ...
    files.forEach((file) => {
      ...
      const childChainIdHex = parseInt(program.opts().childChainId, 10).toString(16).toUpperCase()

      const rootChainIdHex = parseInt(program.opts().rootChainId, 10).toString(16).toUpperCase()

      const data = {
        childChainId: program.opts().childChainId,
        rootChainId: program.opts().rootChainId,
        childChainIdHex: childChainIdHex.length % 2 !== 0 ? `0${childChainIdHex}` : childChainIdHex,
        rootChainIdHex: rootChainIdHex.length % 2 !== 0 ? `0${rootChainIdHex}` : rootChainIdHex
      }
